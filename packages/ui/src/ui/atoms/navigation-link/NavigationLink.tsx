...
export const NavigationLink: React.FC<NavigationLinkProps> = ({
  ...
  return (
    <button
      ...
      <span
        className={tcm(
          `relative pb-1 text-slate-700 dark:text-slate-200 after:transition-transform after:duration-700 after:ease-out after:absolute after:bottom-0 after:left-0 after:block after:h-[2px] after:w-full after:origin-bottom-right ${active ? "after:scale-x-100 font-bold" : "after:scale-x-0"} after:content-[''] after:group-hover:origin-bottom-left after:group-hover:scale-x-100 leading-6 pr-0 after:bg-slate-700/70 dark:after:bg-slate-200/70`,
          spanClassName
        )}>
        <a
          href={active ? undefined : href}
          onClick={active ? (e) => e.preventDefault() : undefined}
          className={`text-slate-700/80 dark:text-slate-200/80 hover:text-slate-700 dark:hover:text-slate-200 ${active ? "font-bold text-slate-700 dark:text-slate-200 cursor-default pointer-events-none" : ""}`}>
          {children}
        </a>
      </span>
    </button>
